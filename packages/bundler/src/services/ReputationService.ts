...
export class ReputationService {
  ...
  async fetchOne(address: string): Promise<ReputationEntry> {
    const raw = await get<ReputationEntrySerialized>(
      this.getKey(address)
    ).catch((_) => null);
    let entry;
    if (!raw) {
  ...
  async dump(): Promise<ReputationEntryDump[]> {
    const addresses: string[] = await get<string[]>(this.REP_COLL_KEY).catch(
      (_) => []
    );
    const rawEntries: ReputationEntrySerialized[] =
      await getMany<ReputationEntrySerialized>(
