...
export class ReqRespRateLimiter {
  private readonly rateLimitersPerPeer = new Map<
    ProtocolID,
    RateLimiterGRCA<string>
  >();
  private readonly rateLimitersTotal = new Map<
    ProtocolID,
    RateLimiterGRCA<null>
  >();
  ...
  initRateLimits<Req>(
    ...
    rateLimits: InboundRateLimitQuota<Req>
  ): void {
    if (!this.enabled) {
      return;
    }
  ...
  allows(peerId: PeerId, protocolID: string, requestCount: number): boolean {
    ...
    if (
      (byPeer && !byPeer.allows(peerIdStr, requestCount)) ||
  ...
  start(): void {
    this.cleanupInterval = setInterval(
      this.checkDisconnectedPeers.bind(this),
  ...
  private checkDisconnectedPeers(): void {
    ...
    for (const [
      peerIdStr,
      lastSeenTime,
    ] of this.lastSeenRequestsByPeer.entries()) {
