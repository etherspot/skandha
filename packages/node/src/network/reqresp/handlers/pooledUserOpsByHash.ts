...
export async function* onPooledUserOpsByHash(
  ...
  const userOpHashes = req.hashes.map(hash => utils.toUtf8String(Bytes32.fromJson(hash)));
  ...
  yield { type: EncodedPayloadType.ssz, data: PooledUserOpsByHash.fromJson(userOps) };
}
