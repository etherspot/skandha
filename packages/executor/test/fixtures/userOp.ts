...
export async function getCounterFactualAddress(
  ...
  try {
    ...
    const initCode = _getAccountInitCode(ownerAddress, salt);
...
async function isAccountDeployed(
  ...
  const provider = new providers.JsonRpcProvider(DefaultRpcUrl);
  const senderAddressCode = await provider.getCode(
    getCounterFactualAddress(ownerAddress, salt)
  );
...
async function _signUserOpHash(
  wallet: Wallet,
  userOpHash: string
): Promise<string> {
