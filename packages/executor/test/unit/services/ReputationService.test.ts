import { describe, it, expect } from 'vitest';
import { BigNumber, utils } from 'ethers';
import { ReputationStatus } from 'types/src/executor';
import { randomAddress } from '../../utils';
import { getClient, getConfigs, getServices } from '../../fixtures';
import { assert } from 'chai';
import * as RpcErrorCodes from 'types/lib/api/errors/rpc-error-codes';

describe('Reputation Service', async () => {
  await getClient();
  it('The fetchOne function with valid ', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();
    const data = await service.fetchOne(wallet.address);
    assert.isNumber(
      data.chainId,
      'The chainId value is not number in the response of fetchone function.'
    );
    assert.isNotEmpty(
      data.address,
      'The address value is displayed empty in the response of fetchone function.'
    );
    assert.isNumber(
      data.lastUpdateTime,
      'The lastUpdateTime value is not number in the response of fetchone function.'
    );
  });

  it('Upadate the seen status with valid details', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();
    await service.updateSeenStatus(wallet.address);
  });

  it('Upadate the included status with valid details', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();
    await service.updateIncludedStatus(wallet.address);
  });

  it('The status of a fresh entry should be OK', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();

    const status = await service.getStatus(wallet.address);
    assert.strictEqual(
      status,
      ReputationStatus.OK,
      'The reputation status is not displayed OK.'
    );
  });

  it('The status of a fresh entry should be BANNED', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();
    await service.setReputation(wallet.address, 1000000000000000, 100);

    const status = await service.getStatus(wallet.address);
    assert.strictEqual(
      status,
      ReputationStatus.BANNED,
      'The reputation status is not displayed BANNED.'
    );
  });

  it('Set the reputation with valid details', async () => {
    let random1 = Math.floor(Math.random() * 10 + 1);
    let random2 = Math.floor(Math.random() * 10 + 1);
    const { service } = await prepareTest();
    const wallet = randomAddress();
    await service.setReputation(wallet.address, random1, random2);

    const data = await service.fetchOne(wallet.address);

    assert.strictEqual(
      data.chainId,
      31337,
      'The chainId value is not displayed correctly in the response of fetchone function.'
    );
    assert.isNotEmpty(
      data.address,
      'The address value is displayed empty in the response of fetchone function.'
    );

    assert.strictEqual(
      data.opsSeen,
      random1,
      'The _opsSeen value is not displayed correctly in the response of fetchone function.'
    );

    assert.strictEqual(
      data.opsIncluded,
      random2,
      'The _opsIncluded value is not displayed correctly in the response of fetchone function.'
    );

    assert.isNumber(
      data.lastUpdateTime,
      'The lastUpdateTime value is not number in the response of fetchone function.'
    );
  });

  it('Dump with single address details', async () => {
    let random1 = Math.floor(Math.random() * 10 + 1);
    let random2 = Math.floor(Math.random() * 10 + 1);
    const { service } = await prepareTest();
    const wallet = randomAddress();
    await service.setReputation(wallet.address, random1, random2);
    const data = await service.dump();

    assert.isNotEmpty(
      data[0].address,
      'The address value is emplty in the dump response.'
    );
    assert.strictEqual(
      data[0].opsSeen,
      random1,
      'The opsSeen value is not displayed equal in the dump response.'
    );
    assert.strictEqual(
      data[0].opsIncluded,
      random2,
      'The opsIncluded value is not displayed equal in the dump response.'
    );
    assert.strictEqual(
      data[0].status,
      0,
      'The status value is not displayed equal in the dump response.'
    );
  });

  it('Validate the crashed handle ops', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();
    await service.crashedHandleOps(wallet.address);
  });

  it('Clear the state of reputation', async () => {
    let random1 = Math.floor(Math.random() * 10 + 1);
    let random2 = Math.floor(Math.random() * 10 + 1);
    const { service } = await prepareTest();
    const wallet = randomAddress();
    await service.setReputation(wallet.address, random1, random2);

    const data = await service.fetchOne(wallet.address);
    assert.strictEqual(
      data.chainId,
      31337,
      'The chainId value is not displayed correctly in the response of fetchone function.'
    );
    assert.isNotEmpty(
      data.address,
      'The address value is displayed empty in the response of fetchone function.'
    );

    assert.strictEqual(
      data.opsSeen,
      random1,
      'The _opsSeen value is not displayed correctly in the response of fetchone function.'
    );

    assert.strictEqual(
      data.opsIncluded,
      random2,
      'The _opsIncluded value is not displayed correctly in the response of fetchone function.'
    );

    assert.isNumber(
      data.lastUpdateTime,
      'The lastUpdateTime value is not number in the response of fetchone function.'
    );

    await service.clearState();

    const clearedData = await service.fetchOne(wallet.address);
    assert.strictEqual(
      clearedData.chainId,
      31337,
      'The chainId value is not displayed correctly in the response of fetchone function.'
    );
    assert.isNotEmpty(
      clearedData.address,
      'The address value is displayed empty in the response of fetchone function.'
    );

    assert.strictEqual(
      clearedData.opsSeen,
      0,
      'The _opsSeen value is not displayed correctly in the response of fetchone function.'
    );

    assert.strictEqual(
      clearedData.opsIncluded,
      0,
      'The _opsIncluded value is not displayed correctly in the response of fetchone function.'
    );

    assert.isNumber(
      clearedData.lastUpdateTime,
      'The lastUpdateTime value is not number in the response of fetchone function.'
    );
  });

  it('Check the stake with undefined', async () => {
    const { service } = await prepareTest();
    const data = await service.checkStake(undefined);

    assert.strictEqual(
      data.msg,
      '',
      'The msg value is not displayed correctly in the response of checkStake function with undefined.'
    );

    assert.strictEqual(
      data.code,
      0,
      'The code value is not displayed correctly in the response of checkStake function with undefined.'
    );
  });

  it('Check the stake with too low stakes', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();
    const data = await service.checkStake({
      stake: utils.parseEther('0.00001'),
      unstakeDelaySec: 2000,
      addr: wallet.address,
    });

    assert.strictEqual(
      data.msg,
      wallet.address +
        ' stake 10000000000000 is too low (min=10000000000000000)',
      'The msg value is not displayed correctly in the response of checkStake function with too low stakes.'
    );

    assert.strictEqual(
      data.code,
      RpcErrorCodes.STAKE_DELAY_TOO_LOW,
      'The code value is not displayed correctly in the response of checkStake function with too low stakes.'
    );
  });

  it('Check the stake with zero stakes', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();
    const data = await service.checkStake({
      stake: 0,
      unstakeDelaySec: 2000,
      addr: wallet.address,
    });

    assert.strictEqual(
      data.msg,
      wallet.address + ' is unstaked',
      'The msg value is not displayed correctly in the response of checkStake function with zero stake.'
    );

    assert.strictEqual(
      data.code,
      RpcErrorCodes.STAKE_DELAY_TOO_LOW,
      'The code value is not displayed correctly in the response of checkStake function with zero stake.'
    );
  });

  it('Check the stake with less than minUnstakeDelay value', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();
    const data = await service.checkStake({
      stake: utils.parseEther('0.1'),
      unstakeDelaySec: -1, // minUnstakeDelay is given in the config file in packages\executor\test\fixtures\getConfig.ts
      addr: wallet.address,
    });

    assert.strictEqual(
      data.msg,
      wallet.address + ' unstake delay -1 is too low (min=0)',
      'The msg value is not displayed correctly in the response of checkStake function with less than minUnstakeDelay value.'
    );

    assert.strictEqual(
      data.code,
      RpcErrorCodes.STAKE_DELAY_TOO_LOW,
      'The code value is not displayed correctly in the response of checkStake function with less than minUnstakeDelay value.'
    );
  });

  it('Check the stake with banned address', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();
    await service.setReputation(wallet.address, 1000000000000000, 100);
    const data = await service.checkStake({
      stake: utils.parseEther('0.1'),
      unstakeDelaySec: 2000,
      addr: wallet.address,
    });

    assert.strictEqual(
      data.msg,
      wallet.address + ' is banned',
      'The msg value is not displayed correctly in the response of checkStake function with banned address.'
    );

    assert.strictEqual(
      data.code,
      RpcErrorCodes.PAYMASTER_OR_AGGREGATOR_BANNED,
      'The code value is not displayed correctly in the response of checkStake function with banned address.'
    );
  });

  it('Check the stake with valid details', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();
    const data = await service.checkStake({
      stake: utils.parseEther('0.01'),
      unstakeDelaySec: 2000,
      addr: wallet.address,
    });

    assert.strictEqual(
      data.msg,
      '',
      'The msg value is not displayed correctly in the response of checkStake function with valid details.'
    );

    assert.strictEqual(
      data.code,
      0,
      'The code value is not displayed correctly in the response of checkStake function with valid details.'
    );
  });

  it('Calculate Max Allowed Mempool Ops Unstaked Check with null value', async () => {
    const { service } = await prepareTest();
    const data = service.calculateMaxAllowedMempoolOpsUnstaked(null);

    assert.isNumber(
      data,
      'The response of calculateMaxAllowedMempoolOpsUnstaked function is not a number.'
    );
  });

  it('Calculate Max Allowed Mempool Ops Unstaked Check with entry', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();
    const data = await service.fetchOne(wallet.address);
    const data2 = service.calculateMaxAllowedMempoolOpsUnstaked(data);

    assert.isNumber(
      data2,
      'The response of calculateMaxAllowedMempoolOpsUnstaked function is not a number.'
    );
  });

  it('Validate the address is blacklistaddress or not', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();
    const data = await service.isBlacklisted(wallet.address);

    if (data === true) {
      assert.ok('The address is blacklisted.');
    } else {
      assert.ok('The address is not blacklisted.');
    }
  });

  it('Add the address into whitelist', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();
    await service.addToWhitelist(wallet.address);
  });

  it('Validate the address without whitelistaddress', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();
    const data = await service.isWhitelisted(wallet.address);

    try {
      if (data === false) {
        assert.ok('The address is not whitelisted.');
      } else {
        assert.fail('The status of the address is not displayed correctly.');
      }
    } catch (error) {
      assert.fail('The status of the address is not displayed correctly.');
    }
  });

  it('Validate the address with whitelistaddress', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();
    await service.addToWhitelist(wallet.address);
    const data = await service.isWhitelisted(wallet.address);

    try {
      if (data === true) {
        assert.ok('The address is whitelisted.');
      } else {
        assert.fail('The status of the address is not displayed correctly.');
      }
    } catch (error) {
      assert.fail('The status of the address is not displayed correctly.');
    }
  });

  it('Fetch the whitelisted address', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();
    await service.addToWhitelist(wallet.address);
    await service.isWhitelisted(wallet.address);
    const data = await service.fetchWhitelist();

    assert.strictEqual(
      data[0],
      wallet.address,
      'The whitelisted address is not displayed correctly.'
    );
  });

  it('Validate the removed address from the whitelistaddress', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();
    await service.addToWhitelist(wallet.address);
    const data = await service.isWhitelisted(wallet.address);

    try {
      if (data === true) {
        await service.removefromWhitelist(wallet.address);

        const check_data = await service.isWhitelisted(wallet.address);

        if (check_data === false) {
          assert.ok('The address is not whitelisted.');
        } else {
          assert.fail('The status of the address is not displayed correctly.');
        }
      } else {
        assert.fail('The status of the address is not displayed correctly.');
      }
    } catch (error) {
      assert.fail('The status of the address is not displayed correctly.');
    }
  });

  it('Add the address into blacklist', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();
    await service.addToBlacklist(wallet.address);
  });

  it('Validate the address without blacklistaddress', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();
    const data = await service.isBlacklisted(wallet.address);

    try {
      if (data === false) {
        assert.ok('The address is not blacklisted.');
      } else {
        assert.fail('The status of the address is not displayed correctly.');
      }
    } catch (error) {
      assert.fail('The status of the address is not displayed correctly.');
    }
  });

  it('Validate the address with blacklistaddress', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();
    await service.addToBlacklist(wallet.address);
    const data = await service.isBlacklisted(wallet.address);

    try {
      if (data === true) {
        assert.ok('The address is blacklisted.');
      } else {
        assert.fail('The status of the address is not displayed correctly.');
      }
    } catch (error) {
      assert.fail('The status of the address is not displayed correctly.');
    }
  });

  it('Fetch the blacklisted address', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();
    await service.addToBlacklist(wallet.address);
    await service.isBlacklisted(wallet.address);
    const data = await service.fetchBlacklist();

    assert.strictEqual(
      data[0],
      wallet.address,
      'The blacklisted address is not displayed correctly.'
    );
  });

  it('Validate the removed address from the blacklistaddress', async () => {
    const { service } = await prepareTest();
    const wallet = randomAddress();
    await service.addToBlacklist(wallet.address);
    const data = await service.isBlacklisted(wallet.address);

    try {
      if (data === true) {
        await service.removefromBlacklist(wallet.address);

        const check_data = await service.isBlacklisted(wallet.address);

        if (check_data === false) {
          assert.ok('The address is not blacklisted.');
        } else {
          assert.fail('The status of the address is not displayed correctly.');
        }
      } else {
        assert.fail('The status of the address is not displayed correctly.');
      }
    } catch (error) {
      assert.fail('The status of the address is not displayed correctly.');
    }
  });
});

async function prepareTest() {
  const { config, networkConfig } = await getConfigs();
  const { reputationService: service } = await getServices(
    config,
    networkConfig
  );
  return { service };
}
