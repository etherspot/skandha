...
export function hexRightPad(hex: string, length: number): string {
  ...
  if (bytes.length > length) {
    throw new Error(`Input is longer than target length of ${length} bytes`);
  }

  const padding = new Uint8Array(length - bytes.length); // fills with zeros
  const padded = ethers.utils.concat([bytes, padding]);
...
export function packUserOp(op: UserOperation): PackedUserOperation {
  ...
  return {
    ...
    initCode:
      op.factory == null ? "0x" : hexConcat([factory, op.factoryData ?? "0x"]),
