...
export class Eth {
  ...
  async sendUserOperation(args: SendUserOperationGasArgs): Promise<string> {
    ...
    if (getUserOpGasLimit(userOp, BigInt(0), 0, this.config.userOpGasLimitMarkup) > BigInt(this.config.userOpGasLimit)) {
      throw new RpcError(
      ...
      if (
        ...
        if (nodeApi) {
          ...
          if (matchingMempool) {
            ...
            const shouldPublish = validationResult.belongsToCanonicalMempool;
