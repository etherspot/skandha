...
import { now } from 'packages/api/src/bundler/utils';
...
export class ReputationEntry implements IReputationEntry {
  ...
  isBanned(minInclusionDenominator: number, banSlack: number) {
    const minExpectedIncluded = Math.ceil(this.opsSeen / minInclusionDenominator);
    return minExpectedIncluded > this.opsIncluded + banSlack;
  }

  isThrottled(minInclusionDenominator: number, throttlingSlack: number) {
    const minExpectedIncluded = Math.ceil(this.opsSeen / minInclusionDenominator);
  ...
  setReputation(opsSeen: number, opsIncluded: number) {
    this._opsSeen = opsSeen;
    this._opsIncluded = opsIncluded;
    this.lastUpdateTime = now();
  }
