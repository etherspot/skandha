import { TracerCall, UserOperationStruct } from 'app/@types';
...
import { IAccount__factory, IAggregatedAccount__factory, IAggregator__factory } from 'app/bundler/contracts';
import { IPaymaster__factory } from 'app/bundler/contracts/factories/IPaymaster__factory';
...
export class UserOpValidationService {
  ...
  async simulateCompleteValidation(userOp: UserOperationStruct, entryPoint: string, codehash?: string): Promise<UserOpValidationResult> {
    ...
    for (let [address, trace] of Object.entries(traceCall.trace)) {
      ...
      // OPCODE RULES
      let violation = trace.violation || {};
      for (let [opcode, count] of Object.entries(violation)) {
        if (
          opcode === 'CREATE2' &&
          Number(count) < 2 &&
          title === 'factory'
        ...
        throw new RpcError(`${title} uses banned opcode: ${opcode}`, RpcErrorCodes.INVALID_OPCODE);
      }
      let value = trace.value || 0;
    ...
    for (let [address, trace] of Object.entries(traceCall.trace)) {
      ...
      if (!validationFailed && trace.storage) {
      ...
      if (validationFailed) {
        const unstaked = !entity || await this.reputationService.checkStake(entity);
        if (unstaked) {
          throw new RpcError(
            `unstaked ${title} entity ${address} accessed slot`,
            RpcErrorCodes.INVALID_OPCODE,
    ...
    for (const call of parsedCalls) {
      ...
      if (call.to.toLowerCase() === paymaster?.addr.toLowerCase()) {
        ...
        if (call.method === 'validatePaymasterUserOp' && call.return?.context !== '0x') {
